$(document).ready(function() {
        let totalAmount = 0;
        let totalItems = 0;
        let menuItems = []; // Variable pour stocker les éléments du menu
    
        async function initState() {
                const response = await fetch('data.json');
                menuItems = await response.json(); // Stocke les éléments dans une variable globale
        ...
        function setAddToCartState() {
                ...
                $minusButton.data('name', $(this).data('name')).data('price', $(this).data('price'));
                $plusButton.data('name', $(this).data('name')).data('price', $(this).data('price'));
        
                $(this).append($minusButton, $counter, $plusButton)
                        .css('background-color', 'hsl(14, 86%, 42%)')
                        .css('border', 'none')
        ...
        function updateTotal() {
                ...
                $("#cartTitle").text(`Your cart (${totalItems})`); // update cart title
                manageCartVisibility();
        }
        ...
        function generateData(desktop, category, name, price) {
                ...
                $addToCartDiv.data('name', name).data('price', price);
        ...
        function showOrderPopup() {
                ...
                $(".cartItems .cart-item").each(function() {
                        ...
                        let itemThumbnail = getThumbnailByName(name); // Obtenir la miniature
                        let $itemDiv = $('<div>', { class: "popup-item" });
                        ...
                        $orderItems.append($thumbnailImg, `
        ...
        function getThumbnailByName(name) {
                let item = menuItems.find(item => item.name === name);
                return item ? item.image.thumbnail : ''; // Retourne la miniature ou une chaîne vide si non trouvé
        }
        ...
        $('.start-new-order').on('click', function() {
                ...
                $(".dataItem").each(function() {
                        ...
                        $addToCartDiv.css({ 'background-color': '', 'border': '', 'justify-content': '' }); // Réinitialiser le style
